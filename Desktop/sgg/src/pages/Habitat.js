...
const Habitat = () => {
  ...
  // Fetch the documents related to "Habitat et Urbanisme" from the backend
  useEffect(() => {
    const themeName = encodeURIComponent('Habitat et Urbanisme');
  ...
  return (
    <div className="container mx-auto my-8 p-4">
      <h2 className="text-3xl font-bold text-center mb-8">Documents pour Habitat et Urbanisme</h2>

      {/* Centering the timeline */}
      <div className="relative border-l-2 border-gray-300 pl-8 mx-auto" style={{ maxWidth: '600px' }}>
        {currentItems.map((doc) => (
          <div key={doc.id} className="mb-12 flex items-center justify-between">
            {/* Date Section */}
            <div className="bg-white shadow-lg rounded-full w-24 h-24 flex flex-col items-center justify-center text-blue-500 border border-gray-300 text-center font-semibold">
              <p className="text-2xl">{new Date(doc.dateDepot).getDate()}</p> {/* Extracting day */}
              <p className="text-sm">{new Date(doc.dateDepot).toLocaleString('fr-FR', { month: 'long' })}</p> {/* Extracting month */}
              <p className="text-sm">{new Date(doc.dateDepot).getFullYear()}</p> {/* Extracting year */}
            </div>

            {/* Description and Link */}
            <div className="ml-8 flex-1 text-right">
              <p className="text-xl font-medium text-gray-700 mb-2">{doc.titre}</p>
